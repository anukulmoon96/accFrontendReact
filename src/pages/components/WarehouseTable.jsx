...
export default function WarehouseTable(props) {
	...
	React.useEffect(() => {
		...
		let url = props.state
			? auth.user.isAdmin
				? auth.warehouseId
					? `${process.env.REACT_APP_USERS_BACKEND_URL}api/users/get_shutter_data?warehouse_id=${auth.warehouseId}&start_date=${props.state.startDate}&end_date=${props.state.endDate}&id=${props.state.id}`
					: `${process.env.REACT_APP_USERS_BACKEND_URL}api/users/get_shutter_data?start_date=${props.state.startDate}&end_date=${props.state.endDate}&id=${props.state.id}`
				: auth.warehouseId &&
				  `${process.env.REACT_APP_USERS_BACKEND_URL}api/users/get_shutter_data?warehouse_id=${auth.warehouseId}&start_date=${props.state.startDate}&end_date=${props.state.endDate}&id=${props.state.id}`
			: auth.user.isAdmin
			? auth.warehouseId
				? `${
				  ...
				  }api/users/get_shutter_data?warehouse_id=${
						auth.warehouseId
				...
				: `${
						process.env.REACT_APP_USERS_BACKEND_URL
				  }api/users/get_shutter_data?start_date=${moment(auth.startDate).format(
						"YYYY-MM-DD",
				  )}&end_date=${moment(auth.endDate).format("YYYY-MM-DD")}`
			: auth.warehouseId &&
			  `${
					process.env.REACT_APP_USERS_BACKEND_URL
			  }api/users/get_shutter_data?warehouse_id=${auth.warehouseId}&start_date=${moment(
					auth.startDate,
			...
			<MuiThemeProvider theme={theme}>
				<MaterialTable
					...
					columns={[
						...
						{
							field: "shutter_open_time",
							title: "Shutter Open Time",
						},
						{
							field: "shutter_close_time",
